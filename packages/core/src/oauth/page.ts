...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Kowork is now connected to ${escapeHtml(provider)}.` : "Kowork is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Kowork couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Kowork couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Kowork.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Kowork</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Kowork couldn't finish connecting to "+PROVIDER+"."):"Kowork couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Kowork."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Kowork is now connected to "+PROVIDER+"."):"Kowork is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// kowork: text wordmark replaces the OpenCode logo geometry
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 140 32" fill="none" aria-label="Kowork" role="img">
        <text x="70" y="26" text-anchor="middle" font-family="var(--oc-font-sans)" font-size="28" font-weight="600" letter-spacing="-0.5" fill="var(--oc-icon-strong)">Kowork</text>
      </svg>`
